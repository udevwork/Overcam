...
extension CameraManager {
    ...
    func initialiseCameraLayer(_ cameraView: UIView) {
        ...
        if let cam = cameraLayer {
            cameraView.layer.addSublayer(cam)
        }
    }
...
extension CameraManager {
    func checkPermissions(_ mediaType: AVMediaType) async throws {
        switch AVCaptureDevice.authorizationStatus(for: mediaType) {
            case .denied, .restricted:
                throw getPermissionsError(mediaType)
            case .notDetermined:
    ...
    func permissionsGranded() -> Bool {
        return AVCaptureDevice.authorizationStatus(for: .video) == .authorized
    }
    
    func animateCameraViewEntrance() {
        if cameraView == nil {
            print("NO cameraView ERROR")
            return
        }
